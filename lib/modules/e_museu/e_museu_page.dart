...
class _EMuseuPageState extends State<EMuseuPage> {
  ...
  static String _normalizar(String texto) {
    const comAcento = 'áàâãéèêíïóôõöúüçñ';
    const semAcento = 'aaaaeeeiioooouucn';
    var saida = texto.toLowerCase();
    for (var i = 0; i < comAcento.length; i++) {
      saida = saida.replaceAll(comAcento[i], semAcento[i]);
    }
    return saida;
  }
  ...
  List<_MuseuItem> get _itensFiltrados {
    final porCategoria = _categoriaSelecionada == 'Todos'
        ? _items
        : _items
              .where((item) => item.category == _categoriaSelecionada)
              .toList();
    final termo = _normalizar(_busca.trim());
    if (termo.isEmpty) return porCategoria;
    return porCategoria.where((item) {
      final alvo = _normalizar(
        '${item.name} ${item.manufacturer} ${item.year} ${item.description}',
      );
      return termo.split(' ').every((parte) => alvo.contains(parte));
    }).toList();
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: Stack(
        children: [
          Positioned.fill(child: _MuseumCarousel(items: _itensFiltrados)),
          Positioned(
            top: 20 * s,
            left: 22 * s,
            child: Material(
              ...
              elevation: 5,
              child: IconButton(
                key: const Key('museum-search-button'),
                icon: Icon(
                  _buscaAberta ? Icons.close : Icons.search,
                  color: Colors.white,
                  size: 34 * s,
                ),
                iconSize: 34 * s,
                padding: EdgeInsets.all(16 * s),
                tooltip: 'Buscar no acervo',
                onPressed: () => setState(() {
              ...
              child: Material(
                ...
                child: TextField(
                  ...
                  decoration: InputDecoration(
                    hintText: 'Buscar peça, fabricante, ano...',
                    hintStyle: TextStyle(
          ...
          Positioned(
            ...
            child: Material(
              ...
              elevation: 5,
              child: IconButton(
                key: const Key('museum-about-button'),
                icon: Icon(
                  Icons.info_outline,
                  color: Colors.white,
                  size: 34 * s,
